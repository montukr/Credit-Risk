...
function CustomerDetailPage({ customer, onBack, onSave, saving }) {
  ...
  const sendAlert = async (tx) => {
    const reason = window.prompt(
      "Enter WhatsApp alert reason:",
      `High risk triggered by a ${tx.category} transaction of ₹${tx.amount}`
    );
    if (!reason) return;

    try {
      await api.post(
        `/admin/customer/${customer.id}/send_alert`,
        null,
        { params: { reason } }
      );
  ...
  return (
    <>
      ...
      {/* TRANSACTIONS + SEND ALERT BUTTON */}
      <div className="card" style={{ marginTop: 16 }}>
        <div className="card-header">Recent transactions</div>
