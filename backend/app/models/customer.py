...
def ensure_customer_for_user(db, user):
    ...
    # atomic upsert to avoid duplicates
    res = col.find_one_and_update(
        {"user_id": str(user["_id"]), "source": "app_user"},
        {"$setOnInsert": doc},
        upsert=True,
        return_document=True,  # Return the existing or inserted doc
    )
    return res
...
def update_customer_aggregates_simple(db, customer):
    ...
    # MerchantMixIndex: distinct categories / total tx
...
# ---------------------------------------------------------
# Admin helpers for portfolio views
# ---------------------------------------------------------
def admin_list_customers(db):
    """
    Return customers from app users with fields needed by the admin dashboard.
    """
    return list(
...
# ---------------------------------------------------------
# Canonical customer risk/score handling
# ---------------------------------------------------------
def get_customer_with_latest_score(db, customer_id: str):
    """
    Return customer doc with correct risk_band / last_score.
    Backfill from risk_scores only if fields missing.
    """
    customer = customers_col(db).find_one({"_id": ObjectId(customer_id)})
    if not customer:
        return None

    # Already has the values → return as-is
    if customer.get("risk_band") is not None and customer.get("last_score") is not None:
        return customer

    # Backfill from history if needed
    ...
    if latest_score:
        if customer.get("risk_band") is None:
            customer["risk_band"] = latest_score.get("risk_band")
        if customer.get("last_score") is None:
