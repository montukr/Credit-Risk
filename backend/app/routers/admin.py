...
# -----------------------------------------------------------
# ⭐⭐ MANUAL WHATSAPP ALERT — SEND FROM ADMIN PANEL ⭐⭐
# -----------------------------------------------------------
@router.post("/customer/{customer_id}/send_alert")
def admin_send_manual_alert(
    customer_id: str,
    ...
    """
    Admin triggers a manual High-Risk WhatsApp alert for a customer.
    """
    ...
    if not user_id:
        raise HTTPException(400, "Customer not linked to user")

    # 2️⃣ Load user for phone number
    ...
    if not phone:
        raise HTTPException(400, "User has no WhatsApp number")

    # 3️⃣ Prepare WhatsApp text
    band = cust.get("risk_band", "High")

    reason_text = reason or f"High risk behaviour detected for user {user['username']}."
    ...
    return {"status": "ok", "sent_to": phone}
