...
def customer_detail(
    ...
    """
    Uses the corrected get_customer_with_latest_score:
    - Never overwrites up-to-date customer.risk_band
    - Only backfills from risk_scores when needed
    """
    customer = get_customer_with_latest_score(db, customer_id)
    return to_str_id(customer) if customer else {}
...
def top_customers(
    ...
    """
    - latest → newest app_user customers
    - flagged → High (or High/Critical if you add Critical)
    - utilisation → highest UtilisationPct
    - cash → highest CashWithdrawalPct
    """
